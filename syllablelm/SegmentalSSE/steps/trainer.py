...
class Trainer:
    ...
    def __init__(self, args):
        ...
        [self.model_seg, self.model_emb], self.optimizer = amp.initialize(models=[self.model_seg, self.model_emb], optimizers=self.optimizer, opt_level=self.args.opt_level)
    ...
    def forward(self, batch):
        if self.args.phase == "pretrain":
            losses = {}
            out = self.model_seg(batch['audio'], padding_mask = batch['padding_mask'], tgt = batch['binary_tgts'], tgt_inner = batch['binary_tgts_inner_mask']) # flattened_logitsshould be flattened, with padded tokens taken out
            # flattened_logits = []
            ...
            # GIGA TODO
            losses['bce_loss'] = torch.tensor([0]).cuda()
            # CRF
            if self.args.crf:
    ...
    def validate_seg(self, valid_loader=None, hide_progress=True):
        ...
        for batch in tqdm(valid_loader, disable=hide_progress):
            out = self.model_seg(batch['audio'], batch['padding_mask'], tgt=batch['binary_tgts'], decode=False)['crf_loss']
            outs.append(out.detach().cpu().item())
        logger.info(f'MEAN: {np.array(outs).mean()}')
        return 0, 0, 0
    
    def validate_seg_2(self, valid_loader=None, hide_progress=True): # GIGA TODO
        if valid_loader == None:
            valid_loader = self.valid_loader
        self.model_seg.eval()
        self.model_emb.eval()
    ...
    def _setup_models(self):
        if self.args.phase in ['train', 'validate'] and self.args.forceAligned_train:
            ...
            #model_seg = segmenter.Segmenter(self.args)  # GIGA TODO
            model_seg = segmenter.Segmenter2(self.args)
        ...
        trainables = [p for p in model_seg.parameters() if p.requires_grad] + [p for p in model_emb.parameters() if p.requires_grad]
