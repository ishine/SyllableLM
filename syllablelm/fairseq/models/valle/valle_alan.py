...
class ValleBlock(nn.Module):
    ...
    def forward(self, x, padding_mask=None, causal_start=0, past=None, **kwargs):
...
class ValleBlockMoE(nn.Module):
    def __init__(
        ...
        self.mlp2 = Mlp(
            in_features=dim,
            hidden_features=int(dim * (mlp_ratio_2 or mlp_ratio)),
            act_layer=act_layer,
            drop=mlp_drop,
        )
        self.post_mlp_dropout = nn.Dropout(post_mlp_drop, inplace=False)
...
class ValleBlockEasyMoE(ValleBlockMoE):
    def forward(self, x, padding_mask=None, causal_start=0, past=None):
        if self.layer_norm_first:
            attn, present = self.attn(self.norm1(x), padding_mask, causal_start, past)
            x = x + self.drop_path(attn)
...
class ValleBlockMaskMoE(ValleBlockMoE):
    def forward(self, x, padding_mask=None, moe_mask=None, causal_start=0, past=None):
        assert moe_mask is not None
        if self.layer_norm_first:
            attn, present = self.attn(self.norm1(x), padding_mask, causal_start, past)
            x = x + self.drop_path(attn)
            r = x
            ...
            x_ = x.new_zeros(x.shape)
            x1 = self.mlp1(x[moe_mask])
            x2 = self.mlp2(x[~moe_mask])
            x_[moe_mask] = x1
            x_[~moe_mask] = x2
            x = x_
...
class ValleAlanBase(BaseFairseqModel):
    def __init__(self, cfg):
        ...
        self.phone_embed_weight = nn.Parameter(torch.empty(self.num_text_tokens, cfg.embed_dim).normal_(0, 1 / math.sqrt(cfg.embed_dim)))
        self.encodec_embed_weight = nn.Parameter(torch.empty(self.num_codec_tokens, cfg.encodec_depth, cfg.embed_dim).normal_(0, 1 / math.sqrt(cfg.embed_dim)))
